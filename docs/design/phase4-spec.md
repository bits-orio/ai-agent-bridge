...
Group A cannot fill `day`, `me.p` or `mk`: no tool among Group A's four (below) reads daytime, darkness, or the asker's own position, so those three keys are left out of the object entirely rather than sent empty. `mk` is Group B for the same reason `me.p` is: the search is centred on the asker's position, so it cannot run until that position is available. Both `me.p` and `mk` start arriving once Group B's dedicated `briefing` op ships. Building `fs` in Group A is a join, not a single read: `list_forces` supplies `n`, `ever` and `on`; `current_research{all}` supplies `res` and `prog`; the service matches the two by force name. `ch` and `ses` cost no RCON trip at all, in either group: `ch` is the same local SQLite read `recent_chat` makes, and `ses` is the `sessions.open` fresh/continuing signal already in service/internal/agent/session.go; neither is one of the four `call` trips below.
...
Group A, service only, no mod release: the briefing is not one rpc call. It is four: `list_forces`, `current_research{all}`, `list_surfaces`, `game_time`, the tools the companion already answers today, each its own `call` op serialized on the single RCON mutex (service/internal/rcon/rcon.go, `mu sync.Mutex`, line 59). `list_players` is not a fifth trip: no key in Group A's payload holds a player list or anything only `list_players` supplies, so a trip that would come back only to be discarded is not made at all. Four trips at the measured 105 ms floor add about 420 ms to every question, not one rpc call's worth. Four trips is still far cheaper than the 2 to 6 second round it exists to prevent, which is why the briefing ships ahead of a dedicated op instead of waiting for one.

The asker's name, force, surface and physical surface cost none of those four trips: they arrive with the question itself, read off the question row the moment it was asked (`player_index` line 57, `force` line 58, `surface` line 87, `physical_surface` line 88, companion-mod/scripts/questions.lua) and already surfaced service side as `askerContext` (service/internal/agent/prompt.go, line 70). The asker's map position and the surface's daytime are the two pieces Group A cannot produce at any price: none of its four tools reads a player's position or a surface's daytime and darkness; `game_time` returns tick, ticks played, hours and connected counts and nothing else (companion-mod/scripts/tools/game_time.lua); no tool in the current inventory returns either, `list_players` included, which is the other half of why dropping it costs the payload nothing. Both wait on Group B.

Group B, with a mod release: a dedicated `briefing` op in companion-mod/scripts/rpc.lua returns the whole snapshot in one reply, one trip instead of four. Its request carries only the question id, `{ op = "briefing", qid = <question id> }`; the companion looks the question up in its own ring (`M.find`, companion-mod/scripts/questions.lua, line 110) for the asker it needs, so the wire never repeats a force or player field the companion already holds. The reply is the usual `{ok=true, r=...}` shape, `r` being the payload above, capped at 16384 bytes by its own `CAPS.briefing` entry beside `CAPS.call` (companion-mod/scripts/rpc.lua, line 36), comfortable headroom over the 400 to 800 token budget below. The `calls` batch op (Decision 9) collapses the same four trips into one round trip too, since it already carries a whole round's tool calls in one command and returns an array of results. Whichever of the two a release ships first is what the briefing runs on from then on. The request and reply envelope for both belong to phase4-spec.md section 14; this section specifies only the payload itself, `r`.
...
The payload itself is capped at 16384 bytes, in Group A the same as Group B: the same ceiling the companion's own `CAPS.briefing` entry enforces once the dedicated op ships (section 14), applied service side in the meantime so a Group A payload never grows past what a Group B one is allowed to. Far more than the 400 to 800 token budget above ever needs, but a real ceiling rather than an assumed one. When an assembled payload would cross it, `ch` is the key dropped first: the last five chat lines are the one field a later tool call, `recent_chat`, already answers on its own, so they are the cheapest thing to give up, and every other key survives until `ch` alone is not enough. When it is not, `fs` goes whole, never row by row. A short force list asserts that the forces it leaves out do not exist, and the free tier answers "how many teams are there" from this key with no tool call, so a silently trimmed list tells the same lie a failed `current_research` trip would. An absent key sends the model to the tool; a short one does not.
...
Who is online, by name, is a different question from how many: `fs` carries a connected count per force (`on`), never a player name, so a how-many question is free and a who-by-name question is not. Naming who is online still costs one lookup, `list_players`, the tool this design drops from the Group A fetch precisely because it cannot answer for free what the payload does not carry.
...
briefing (the per-question snapshot) is assembled from four ordinary `call`
trips to tools the companion already answers, `list_forces`,
`current_research{all}`, `list_surfaces`, `game_time`,
serialized on the same mutex as any other round: four trips at the roughly
105 ms floor, about 420 ms added to every question. `briefing` collapses that
to one trip directly; the `calls` op above collapses it to one trip too, by
carrying the same four calls in a single batch, so whichever ships first
removes the four serialized trips.
...
asker's surfaces, the nearest map markers, the last five chat lines). A server
running a companion old enough to have neither `briefing` nor `calls` gets no
dedicated cap at all: its briefing costs four ordinary `call` trips against
the caps this section already sets, and nothing new needs a name.
