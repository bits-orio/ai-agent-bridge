...
| `fs` | array of `{ n, ever, on, res, prog }` | one row per force with a player ever or currently researching: name (`n`), players ever (`ever`), players connected now (`on`), the technology being researched (`res`), its progress 0 to 1 (`prog`); `res` and `prog` are both left out on a force with no active research | Group A |
...
`me` carries no `ps` here because the asker is not in remote view, the same condition under which the question row itself leaves `physical_surface` unset (companion-mod/scripts/questions.lua, line 71). `south`'s row carries no `res` or `prog` because that force has no active research, the same condition `current_research` reports as `researching = false` (companion-mod/scripts/tools/basics.lua, `research_row`).
...
Group A, service only, no mod release: the briefing is not one rpc call. It is five: `list_forces`, `current_research{all}`, `list_surfaces`, `list_players`, `game_time`, the tools the companion already answers today, each its own `call` op serialized on the single RCON mutex (service/internal/rcon/rcon.go, `mu sync.Mutex`, line 59). Five trips at the measured 105 ms floor add about 525 ms to every question, not one rpc call's worth. Five trips is still far cheaper than the 2 to 6 second round it exists to prevent, which is why the briefing ships ahead of a dedicated op instead of waiting for one.

The asker's name, force, surface and physical surface cost none of those five trips: they arrive with the question itself, read off the question row the moment it was asked (`player_index` line 57, `force` line 58, `surface` line 87, `physical_surface` line 88, companion-mod/scripts/questions.lua) and already surfaced service side as `askerContext` (service/internal/agent/prompt.go, line 70). The asker's map position and the surface's daytime are the two pieces Group A cannot produce at any price: `list_players` returns only name, connected and admin, never a position (companion-mod/scripts/tools/basics.lua); `game_time` returns tick, ticks played, hours and connected counts, never daytime or darkness (companion-mod/scripts/tools/game_time.lua); no other tool in the current inventory returns either. Both wait on Group B.
...
Failure path: the briefing is best effort and never fails a question. On timeout, an error, or a companion that lacks the op it needs, the service omits the briefing, logs one line, and the question proceeds exactly as it does without one, the model working from the tools it already has, one round longer than it would have needed. The briefing carries its own budget inside the question's overall one, 2 seconds, tracked separately from the rpc call's own timeout so a slow briefing cannot eat into the rounds that follow it. It is a read either way: it never writes storage, so CONTEXT.md invariant 2 holds regardless of which group produced it.
...
|---|---|---|
| who's online | `fs`: connected players | Group A |
...
`current_research{all}`, `list_surfaces`, `list_players`, `game_time`,
...
dedicated cap at all: its briefing costs five ordinary `call` trips against
