...
| `briefing_ms` | recorded whenever the briefing was attempted, on `on` AND on `failed`, zero only on `off`: wall clock spent fetching the briefing, before round 1. A failed attempt still spent its trips, and reporting it as zero hides a real cost: on 2026-09-13 the briefing failed on every question against a live server and the ledger showed it costing nothing, leaving one log line as the only evidence. It is four serialized `call` trips today at about 420 ms at the 105 ms RCON floor, or the one `briefing` trip once it ships |
