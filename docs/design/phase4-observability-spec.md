...
| `corrected_from` | present only when the model named a tool that does not exist and exactly one tool could have been meant, so the service called that one: `name` is what ran, this is what was asked. Questions 78 and 86 on 2026-09-15 each invented a different provider prefix on the bare `catch_up` and paid a round for it; counting this field says how often that still happens |
...
| `briefing_ms` | recorded whenever the briefing was attempted, on `on` AND on `failed`, zero only on `off`: wall clock spent fetching the briefing, before round 1. A failed attempt still spent its trips, and reporting it as zero hides a real cost: on 2026-09-13 the briefing failed on every question against a live server and the ledger showed it costing nothing, leaving one log line as the only evidence. It is five serialized `call` trips today at about 525 ms at the 105 ms RCON floor, or the one `briefing` trip once it ships |
